...
async function init() {
  ...
  if (!count.c) {
    ...
    const a = await one(`insert into users(org_id,username,password,name,role) values($1,$2,$3,$4,'admin') returning id`, [blanc.id, 'blancadmin', hash, '블랑 관리자']);
    const b = await one(`insert into users(org_id,username,password,name,role) values($1,$2,$3,$4,'admin') returning id`, [flow.id, 'flowcelladmin', hash, '플로셀 관리자']);
    for (const userId of [a.id, b.id]) {
      await q('insert into user_organizations(user_id,org_id) values($1,$2) on conflict do nothing', [userId, blanc.id]);
      await q('insert into user_organizations(user_id,org_id) values($1,$2) on conflict do nothing', [userId, flow.id]);
    }
  }
}
...
app.put('/api/products/:id', auth, adminOnly, async (req, res, next) => {
  try {
...
app.delete('/api/products/:id', auth, adminOnly, async (req, res, next) => {
  try {
    const product = await ownProduct(req.params.id, req.session.user.orgId);
    if (!product) return res.status(404).json({ message: '제품을 찾을 수 없습니다.' });
...
app.post('/api/users', auth, adminOnly, async (req, res, next) => {
  const client = await pool.connect();
  try {
    const { username, password, name, role = 'staff', orgIds } = req.body;
    const ids = Array.isArray(orgIds) ? orgIds.map(Number).filter(Boolean) : [Number(req.session.user.orgId)];
    ...
    await client.query('begin');
    const created = await client.query(`insert into users(org_id,username,password,name,role) values($1,$2,$3,$4,$5) returning id`, [ids[0], username.trim(), bcrypt.hashSync(password, 10), name.trim(), role === 'admin' ? 'admin' : 'staff']);
    ...
    await client.query('commit');
    await log(req, '직원 계정 추가', '직원', name.trim(), `${username.trim()} · ${ids.length === 2 ? '두 사업장 사용' : '선택 사업장 사용'}`);
    res.json({ ok: true });
  } catch (e) {
